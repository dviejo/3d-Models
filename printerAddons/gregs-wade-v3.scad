...
// Adapted for reprap UA Workshop by dviejo
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
wildseyed_mount_jHead = 3;
wildseyed_mount_e3d = 3.8;
wildseyed_mount_head = wildseyed_mount_e3d;
...
//translate([52,55,0]) bearing_washer();
...
wade_block_width=24;
wade_block_depth=28; //original 28 - Ojo! No escala bien!!!
...
motor_mount_translation=[46.0,54.0,0];
	...
	mounting_holes=default_mounting_holes)
{
	difference ()
	{
		union()
		{
			...
			//Base aligement helper --- removed by dviejo
...
/*	translate([-24.5+64+4,-1,3]) {
		translate([-24,0,0]) { //-46
			translate([0,0,(wade_block_depth+base_extra_depth)/2+4+layer_thickness]) cylinder(r=m3_diameter/2, h=wade_block_depth+base_extra_depth, center=true,$fn=20);
			cylinder(r=m3_nut_diameter/2, h=8, center=true,$fn=20);
		}
		
		translate([0,0,0]) { //-22
			translate([0,0,(wade_block_depth+base_extra_depth)/2+4+layer_thickness]) cylinder(r=m3_diameter/2, h=wade_block_depth+0.2+base_extra_depth, center=true,$fn=20);
			cylinder(r=m3_nut_diameter/2, h=8, center=true,$fn=20);
		}
	}
*/
...
{  
	extruder_recess_d=insulator_d+0.6; //0.7
	extruder_recess_h=10;
	...
	for (hole=[-1,1])
	  rotate(90,[1,0,0])
	    translate([hole*(extruder_recess_d/2-1.5),wildseyed_mount_head+1.5,-wade_block_depth/2-1])
	      cylinder(r=1.5,h=wade_block_depth+2,$fn=10);
}
